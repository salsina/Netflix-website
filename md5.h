...
private:
    ...
    static const array<std::uint32_t, 64> k_array_;
    static const array<std::uint32_t, 64> s_array_;

private:
    static uint32_t left_rotate(std::uint32_t x, std::uint32_t c) {
        return (x << c) | (x >> (32 - c));
    }
...
private:
    ...
    void bytes_to_m_array(InputIterator & first, std::array<uint32_t, 16>::iterator m_array_last) {
...
public:
    ...
    void update(InputIterator first, InputIterator last) {
        ...
        uint64_t original_length_bits = std::distance(first, last) * 8;
